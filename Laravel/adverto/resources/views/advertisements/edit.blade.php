    ...
    <div class="md:focus:ring-0 md:w-auto w-screen flex-col items-center md:p-10">
        ...
        <form action="{{ route('advertisements.update', $advertisement->id) }}" method="POST" class="" enctype="multipart/form-data">
            ...
            <div class="mb-4 bg-white p-8 rounded-md">
                ...
                <input id="user_autocomplete_address" name="user_autocomplete_address" class="w-full {{ $errors->has('user_autocomplete_address') ? 'border-b-2 border-red-500 border-x-0 border-t-0' : 'border-transparent' }} rounded px-4 py-2 bg-[#f2f4f5]" value="{{$advertisement->location->city}}" placeholder="Zacznij wpisywać swój adres...">
            ...
            <input id="locality" name="locality" value="{{$advertisement->location->city}}" type="hidden">
            <input id="administrative_area_level_1" name="administrative_area_level_1" value="{{$advertisement->location->province}}" type="hidden">
            <input id="country" value="{{$advertisement->location->country}}" name="country" type="hidden">
